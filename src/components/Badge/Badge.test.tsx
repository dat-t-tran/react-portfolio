...
describe('Badge', () => {
    ...
    it('applies custom class to the badge', () => {
        ...
        const { container } = render(<Badge description='Test Badge' customClass={customClass} />)
    ...
    it('renders the correct star icon based on screen size', () => {
        ...
        if (pictureElement) {
            ...
            expect(sourceElements[0]).toHaveAttribute('srcSet', assets.starSm)
            expect(sourceElements[0]).toHaveAttribute('media', '(min-width: 800px)')
            expect(sourceElements[0]).toHaveAttribute('type', 'image/svg')

            expect(sourceElements[1]).toHaveAttribute('srcSet', assets.star)
            expect(sourceElements[1]).toHaveAttribute('media', '(min-width: 600px)')
            expect(sourceElements[1]).toHaveAttribute('type', 'image/svg')

            expect(imgElement).toHaveAttribute('src', assets.starSm)
            expect(imgElement).toHaveAttribute('alt', 'Star icon')
