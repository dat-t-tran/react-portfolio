...
describe('HeroSection', () => {
    it('renders the HeroSection component', () => {
        ...
        expect(screen.getByText((_, element) => {
            const hasText = (node: Node) => node.textContent === 'Connecting You to Better Health';
        ...
        expect(screen.getByText((_, element) => {
            const hasText = (node: Node) => node.textContent === "We're here to link you directly to improved health outcomes, effortlessly connecting you with the care you need.";
            const nodeHasText = element ? hasText(element) : false;
            const childrenDontHaveText = Array.from(element?.children || []).every(child => !hasText(child));
            return nodeHasText && childrenDontHaveText;
        })).toBeInTheDocument();
